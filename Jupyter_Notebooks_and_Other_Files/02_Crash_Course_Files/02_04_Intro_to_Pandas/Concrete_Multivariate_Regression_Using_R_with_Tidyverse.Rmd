...
-   Install Us First
    ...
    -   [moderndive](Tidyverse-Friendly%20Introductory%20Linear%20Regression) : Tidyverse-Friendly Introductory Linear Regression
...
To crack open the spreadsheet we will want to use the [readxl::read_excel](https://www.rdocumentation.org/packages/readxl/versions/1.1.0/topics/read_excel) function.
...
![Example of the "pivot_long" Function](https://epirhandbook.com/en/images/pivoting/pivot_longer_new.png)

![Example of the "pivot_wide" Function](https://epirhandbook.com/en/images/pivoting/pivot_wider_new.png)

In R this is now done with an analog to a function in [Python's Pandas's package (and excel), the "pivot"](https://pandas.pydata.org/docs/user_guide/reshaping.html).

In R this is managed in the [tidr](https://tidyr.tidyverse.org/articles/pivot.html) package using [tidyr::pivot_longer()](https://tidyr.tidyverse.org/reference/pivot_longer.html) to gathers a group of columns into a single longer column, and [tidyr::pivot_wider()](https://tidyr.tidyverse.org/reference/pivot_wider.html) which spreads a column into a shorter-but-wider table.
...
We can actually do this with a correlate function, [boot::cor()](https://www.rdocumentation.org/packages/stats/versions/3.4.3/topics/cor)...
...
You also can graph the look-n-feel of what all of the different correlations are... (it works best with a much smaller number of variables).  This function is called [corrplot::corrplot()](https://cran.r-project.org/web/packages/corrplot/vignettes/corrplot-intro.html) and makes a nice presentation of the data.
...
Try [graphics::pairs()](https://www.rdocumentation.org/packages/graphics/versions/3.5.1/topics/pairs)
...
Here we will use the [stats::lm()](https://www.rdocumentation.org/packages/stats/versions/3.4.3/topics/lm) (linear model) from the basic toolboxes that come with R.
...
For this we use an extension to the graphics package called [ggplot2::geom_smooth()](https://ggplot2.tidyverse.org/reference/geom_smooth.html)
...
I'm using the get_regression_points function which adds the modeled "y-hat" value to the dataframe of all of the other values [stats::get_regression_points()](https://www.rdocumentation.org/packages/stats/versions/3.5.1/topics/fitted) function.
